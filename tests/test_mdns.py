...
def test_build_query_sets_qu_bit_and_question():
    q = mdns.build_query("K4-SN00895.local", qid=0x1234)
    qid, _flags, qd, an, _ns, _ar = struct.unpack(">HHHHHH", q[:12])
    assert qid == 0x1234 and qd == 1 and an == 0
    assert b"\x0aK4-SN00895\x05local\x00" in q      # length-prefixed labels
...
def _response(ip: str) -> bytes:
    ...
    qname = b"\x0aK4-SN00895\x05local\x00"
...
def test_parse_response_extracts_ip():
    assert mdns.parse_response(_response("10.0.1.105"), "K4-SN00895.local") == "10.0.1.105"
